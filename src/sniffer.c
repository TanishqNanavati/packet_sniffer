...
int Sniffer_init(Sniffer* s, const char* interface, const SimpleFilter* filter, unsigned int count) {
    ...
    if (filter)
        ...
        memset(&s->filter, 0, sizeof(SimpleFilter));
...
static int setup_socket(Sniffer* s) {
    ...
    if (setsockopt(s->sockfd, SOL_PACKET, PACKET_ADD_MEMBERSHIP,
                   &mreq, sizeof(mreq)) == -1) {
        fprintf(stderr, "Warning: could not enable promiscuous mode: %s\n",
                strerror(errno));
    }
...
/* ===========================================================
   Process Packet — updated counter logic
   =========================================================== */
static void process_packet(Sniffer* s, const uint8_t* buffer, ssize_t len) {

    s->total++;   /* Count everything (before filter!) */

    print_eth(buffer, len);
    ...
    if (len < (ssize_t)(sizeof(struct EthHeader) + sizeof(struct IPv4Header)))
        return;
    ...
    switch (ip->protocol) {
        case IPPROTO_TCP:
            s->tcp++;
            print_tcp(buffer, len, ihl);
            break;

        case IPPROTO_UDP:
            s->udp++;
            print_udp(buffer, len, ihl);
            break;

        case IPPROTO_ICMP:
            s->icmp++;
            print_icmp(buffer, len, ihl);
            break;
...
/* ===========================================================
   Start Sniffer — updated for clean shutdown
   =========================================================== */
int Sniffer_start(Sniffer* s) {
    ...
    while (g_running) {
        ...
        ssize_t len = recvfrom(s->sockfd, buffer, 65536, 0, NULL, NULL);
        ...
        /* filter check */
        if (!packet_matches_filter(buffer, len, &s->filter))
            continue;
